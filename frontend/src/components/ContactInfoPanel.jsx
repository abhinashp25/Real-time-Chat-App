...
export default function ContactInfoPanel({ user, onClose, onClearChat, onArchive }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center gap-4 px-5 h-16 sm:h-[68px] flex-shrink-0" style={{ background: "#202c33", color: "white" }}>
      ...
      <div className="flex-1 overflow-y-auto" style={{ background: "#0b141a" }}>
        ...
        {/* Profile Card */}
        <div className="flex flex-col items-center py-8 px-6 bg-[#111b21] shadow-sm mb-2">
          ...
          <p className="text-[15px] text-[#8696a0] mt-1">{user.email}</p>
          <p className="text-[14px] mt-1" style={{ color: isOnline ? '#00a884' : '#8696a0' }}>
            {isOnline ? "online" : lastSeenLabel(lastSeen)}
          </p>

          <div className="flex w-full items-center justify-center gap-8 mt-6">
            <button className="flex flex-col items-center gap-2 text-[#00a884] hover:opacity-80 transition-opacity">
              <div className="p-3 bg-[#0b141a] rounded-xl"><PhoneIcon size={24} /></div>
              <span className="text-[13px] font-medium">Audio</span>
            </button>
            <button className="flex flex-col items-center gap-2 text-[#00a884] hover:opacity-80 transition-opacity">
              <div className="p-3 bg-[#0b141a] rounded-xl"><VideoIcon size={24} /></div>
              <span className="text-[13px] font-medium">Video</span>
            </button>
            <button className="flex flex-col items-center gap-2 text-[#00a884] hover:opacity-80 transition-opacity">
              <div className="p-3 bg-[#0b141a] rounded-xl"><XIcon className="rotate-45" size={24} /></div>
        ...
        {/* About Section */}
        <div className="bg-[#111b21] px-5 py-4 shadow-sm mb-2">
        ...
        <button className="w-full flex items-center justify-between bg-[#111b21] px-5 py-4 shadow-sm mb-2 hover:bg-[#202c33] transition-colors">
        ...
        {/* Config Section 1 */}
        <div className="bg-[#111b21] shadow-sm mb-2">
          <button 
            className="w-full flex items-center justify-between px-5 py-4 hover:bg-[#202c33] transition-colors"
            onClick={() => setNotifications(!notifications)}
          >
            <div className="flex items-center gap-4">
              <div className="text-[#8696a0]">{notifications ? <BellIcon size={20} /> : <BellOffIcon size={20} />}</div>
          ...
          <button className="w-full flex items-center justify-between px-5 py-4 hover:bg-[#202c33] transition-colors">
            <div className="flex items-center gap-4">
              <div className="text-[#8696a0]"><StarIcon size={20} /></div>
              <span className="text-white text-[15px]">Starred messages</span>
            </div>
            <ChevronRightIcon size={16} className="text-[#8696a0]" />
          </button>
        </div>

        {/* Config Section 2 */}
        <div className="bg-[#111b21] shadow-sm mb-2">
          <button className="w-full flex flex-col items-start px-5 py-4 hover:bg-[#202c33] transition-colors">
            <div className="flex items-center gap-4 mb-1">
              <div className="text-[#8696a0]"><TimerIcon size={20} /></div>
              <span className="text-white text-[15px]">Disappearing messages</span>
            </div>
            <p className="text-[#8696a0] text-[13px] pl-9">Off</p>
          </button>
        </div>

        {/* Danger Actions */}
        <div className="bg-[#111b21] shadow-sm mb-6">
          <button className="w-full flex items-center gap-4 px-5 py-4 hover:bg-[#202c33] transition-colors text-[#f15c6d]">
          ...
          {onClearChat && (
            <button className="w-full flex items-center gap-4 px-5 py-4 hover:bg-[#202c33] transition-colors text-[#f15c6d]" onClick={onClearChat}>
              <XIcon size={20} />
